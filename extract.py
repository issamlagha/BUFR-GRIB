...
# Définir les coordonnées du domaine géographique
    ...
    Lat1 = 29.710409738215581  # Latitude minimale

    Lat2 = 45.431041359817520  # Latitude maximale

    Lon1 = -6.054430658010459  # Longitude minimale

    Lon2 = 14.231347490178145  # Longitude maximale
...
#    BlockNum = codes_get_long(bufr, 'blockNumber')
    ...
    #try:
        ...
        alt = codes_get_array (bufr , 'heightOfStation' )
        time = codes_get (bufr , 'timePeriod' )
        day = codes_get(bufr , 'typicalDate' )
        hour = codes_get(bufr , 'typicalTime' )
        value = codes_get_array (bufr , 'atmosphericPathDelayInSatelliteSignal')
    #print( lat )  
    #print(lon)
#     print(BlockNum)
    #print(StatNum)
...
repertoire = '/scratch/work/mokhtari/AROME_ASSIM/OBS/GNSS'



# Date de début (premier du mois à 00:00:00)

date_debut = datetime(2023, 5, 1, 0, 0)



# Date de fin (quinze du mois à 23:00:00)

date_fin = datetime(2023, 5, 15, 23, 0)
...
#    nom_fichier = date_actuelle.strftime('%Y%m%d%H.bufr')
    nom_fichier = date_actuelle.strftime('gpssol_%Y%m%d%H00.bufr')
