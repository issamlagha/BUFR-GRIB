...
files = sorted(glob(os.path.join(BASE_DIR, "*AFA*BODY*0001.nc")))
...
for f in files:
    try:
        ...
        scn.load(["accumulated_flash_area"])
        
        # Generate output path safely
        timestamp = datetime.strptime(os.path.basename(f).split("_")[4], "%Y%m%d%H%M%S")
        output_path = os.path.join(OUTPUT_DIR, f"accumulated-flash-area_{timestamp:%Y%m%d-%H%M%S}.tif")
        
        scn.save_dataset("accumulated_flash_area", 
